...
<strong>自然にできる小さなドームを使う（自己組織化成長法）</strong>： ある材料の上に別の材料をのせると、小さなドームのようなものが自然にできます。葉っぱの上に水滴を落とすと、小さな水玉ができるのと似ています。この小さなドームの中に電子を閉じ込めることで、量子ドットを作ることができます。

<strong>通行止にする方法（リソグラフィー技術）</strong>： まず、半導体の表面に特別な模様（パターン）を作ります（これをリソグラフィー技術といいます）。その形を使って、電子が入れる場所と入れない場所を作り出します。これは、道路を作って通行できる道を決めるのと似ています。電子が進める場所を作り、他の場所には壁を作ることで、電子を特定の領域に閉じ込めることができます。
